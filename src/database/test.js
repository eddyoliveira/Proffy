...
Database.then(async (db) => {
    ...
    // o horário que a pessoa trabalha, por exemplo, é das 08 - 18h
    // o horário do time_from (0h) precisa ser menor ou igual ao horário solicitado
    // o time_to precisa ser maior
    const selectedClassesSchedules = await db.all(`
        SELECT class_schedule.* FROM class_schedule WHERE class_schedule.class_id = "1"
        AND class_schedule.weekday = "0"
        AND class_schedule.time_from <= "520"
        AND class_schedule.timo_to > "520"
    `)
